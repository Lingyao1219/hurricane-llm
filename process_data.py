...
def main(folder, language):
    ...
    for file in files:
        ...
        df1['image_urls'], df1['image_hashes'], df1['unique_image_urls'] = zip(*df1.progress_apply(lambda row: combine_urls(row, seen_hashes), axis=1))
        df2 = df1[df1['unique_image_urls'].apply(lambda x: len(x) > 0)]
