...
def main(read_folder, save_folder):
    ...
    for file in files:
        ...
        chunk_size = 10000
        num_chunks = len(tweets_data) // chunk_size + (1 if len(tweets_data) % chunk_size != 0 else 0)
        chunk_list = []
        for i in range(num_chunks):
            start = i * chunk_size
            end = (i + 1) * chunk_size
            chunk = tweets_data[start:end]
            df_chunk = convert_dataframe(chunk)
            df_chunk['text'] = df_chunk.apply(lambda row: convert_text(str(row['text']), str(row['rt_text'])), axis=1)
            chunk_list.append(df_chunk)
        df = pd.concat(chunk_list).reset_index(drop=True)
        save_filepath = os.path.join(save_folder, file)
        df.to_json(save_filepath)
        print(f'Finish converting {file} and save to {save_filepath}.')
